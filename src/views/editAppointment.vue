    ...
    methods:{
        ...
        sendEdited:function(){
            ...
            for(var i=0;i<this.editing.cohorts.length;i++){
                this.edited.cohorts.push({month:this.editing.cohorts[i].month,
                year:this.editing.cohorts[i].year
                }
                )
            }
            for(var k=0;k<this.edited.cohorts.length;k++){
                for(var j=0;j<this.newCohorts.length;j++){
                    if(this.edited.cohorts[k].month==this.newCohorts[j].month && this.edited.cohorts[k].year==this.newCohorts[j].year){
                       this.duplicates.push({new:this.newCohorts[j],old:this.edited.cohorts[k]})
                    }
                }
            }
